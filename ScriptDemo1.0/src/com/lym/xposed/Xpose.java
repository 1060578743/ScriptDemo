...
public class Xpose implements IXposedHookLoadPackage {
	...
	public void onToastShow(Toast toast) {
		...
		XposedBridge.log(ViewUtil.getAllText(view));
		// LogUtil.log(ViewUtil.dump(view));
		// 如果包含root提示,则取消显示
		...
		XConnection.getInstance().onToastgShow(toast);
	}

	public void onDialogShow(Dialog dialog) {
		XConnection.getInstance().onDialogShow(dialog);
	}

	public void onActivityonResume(Activity act) {
		XConnection.getInstance().onActivityResume(act);
	}

	public void onActivityPause(Activity act) {
		XConnection.getInstance().onActivityPause(act);
	}
